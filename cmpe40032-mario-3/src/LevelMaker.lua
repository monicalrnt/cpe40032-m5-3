...
function LevelMaker.generate(width, height)
	...
	-- grp3.2 - Key and lock locations randomly generated
	...
	for x = 1, width do
		...
		-- grp3.1 - the game already checked to make sure column 1 has solid ground
		...
		if (math.random(10) == 1) and (x ~= keyLocation) and (x ~= lockLocation) and (x < width - 3) then
			...
			-- grp3.2 - Logic for block containing key
						...
						onCollide = function(player, object)
							...
							if player.key > 0 then
								local pole = GameObject {
									...
									-- grp3.3 - Goal shifts to next level
			...
			elseif (x < width - 1 and math.random(10) == 1) and (x ~= keyLocation + 1) and (x ~= lockLocation + 1) then
				-- grp3 - chance to spawn gem reliant on type of block
				...
				table.insert(objects,
					...
					GameObject {
						...
						onCollide = function(player, obj)
							...
							-- grp3 - Chance for gems and others on non-exhausted blocks
							if (not obj.hit) and (obj.frame % 2 == 1) then

								-- grp3 - Chance for snail
